...
fileitem = form['TABLE_REPORT1']
db=MySQLdb.connect("localhost", "root","root","REPORT_AUTOMATION")
db1=db.cursor()
sql1="SELECT TABLE_REPORT, SAMPLE_TYPE FROM REPORT WHERE SO_NUMBER='{}'".format(SO_NUMBER)
db1.execute(sql1)
...
if String_Value[0][0] !='YES':
    ...
    sql="UPDATE REPORT SET TABLE_REPORT = 'YES' WHERE SO_NUMBER = '{}';".format(SO_NUMBER)
    db1.execute(sql)
    db.commit()
