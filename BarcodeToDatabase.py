...
SO_NUMBER = ((form["hidden"].value).upper()).rstrip()
db=MySQLdb.connect("localhost", "root", "root", "Tentative_plan")
db1= db.cursor()
db1.execute("""SELECT ID FROM plan WHERE SO_NUMBER='{}';""".format(SO_NUMBER))
SO_ID=sorted(db1.fetchall())
...
for i,j in enumerate(SO_ID):
    onename=form["BARCODE_NAME1{}".format(i+1)].value
    oneseq= Barcode_Seq_Lis[Barcode_Key_Lis.index(onename)]
    db1.execute(upd_sql.format(onename,oneseq,j[0]))
...
print("""<p>BARCODING_FOR__THE_SAMPLES_OF__SO_NUMBER_{0}__UPLOADED<p> <meta http-equiv="refresh" content="2;url=http://localhost/UploadFile.py?SO_NUMBER={0}" /> Redirecting... </body> </html></body><html>""".format(SO_NUMBER))
