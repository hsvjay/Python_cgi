...
def selectPeople(db, db1):
    sql= "SELECT * FROM plan where Machine_Type='{}';"
    db1.execute(sql.format(MACHINE_TYPE))
    rows= db1.fetchall()
    summ=0.0
    people=[]
    for row in rows:
...
def backup_Runnedplan(people):
    print """<form action="/cgi-bin/Runned_plan.py" method="POST">"""
    print "<br><br><table border= '1'>"
    print """<tr><td>BACKUP_RUNNED_PLAN:::</td><td><input type="hidden" name="MACHINE_TYPE" value='{}'>
<input type="hidden" name="SUM" value={}></td><td><input type="submit" value="SUBMIT" /></td></tr></table></form>""".format(MACHINE_TYPE, SUM)
...
if __name__ == "__main__":
    try:
        ...
        print "<hr><hr><hr><br>Please select the machine_type<br><br><hr><hr><hr>"
        print '</body>'
        print '</html>'
