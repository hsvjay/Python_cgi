...
CHEMISTRY =((form["CHEMISTRY"].value).upper()).rstrip()
Machine_Type =((form["Machine_type"].value).upper())
...
if not SO_NUMBER in [row[1] for row in db1.fetchall()]:
    for i in range(NO_OF_SAMPLE):
        ...
        db.commit()
    print '<form action="/cgi-bin/testBarcode.py" method="POST">'
    ...
    print '</form>'
    print '</body><html>'   
    '''print """ <p>THE SO_NUMBER '{}' ASSIGNED FOR THE CLIENT '{}', IS REPOSITED TO THE DATABASE NAMED TENTATIVE_PLAN</p> <meta http-equiv="refresh" content="2;url=http://localhost/common.cgi?SO_NUMBER={}" /> Redirecting... </body> </html>""".format(SO_NUMBER,CLIENT_NAME,SO_NUMBER)'''
else:
     print """ <p>THE SO_NUMBER %s ASSIGNED FOR THE CLIENT %s UR TRYING TO, IS ALREADY IN THE DATABASE NAMED TENTATIVE_PLAN</p>  </body>
